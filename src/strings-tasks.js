...
function concatenateStrings(value1, value2) {
  const concatStr = '';
  return concatStr.concat(value1, value2);
}
...
function isPalindrome(str) {
  const lowStr = str.toLowerCase().replace(/[^a-z0-9а-яієїґ]/gi, '');
...
function reverseWords(str) {
  const reverseAllWord = [];
  const arrStr = str.split(' ');
  for (let i = 0; i < arrStr.length; i += 1) {
    reverseAllWord.push(arrStr[i].split('').reverse().join(''));
  }
  return reverseAllWord.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arrLetters.length; i += 1) {
    if (arrLetters[i] === arrLetters[i].toLowerCase()) {
      arrLetters[i] = arrLetters[i].toUpperCase();
      newStr.push(arrLetters[i]);
    } else {
      newStr.push(arrLetters[i].toLowerCase());
    }
  }
  return newStr.join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const index = firstStr.indexOf(str[i]);
    if (index !== -1) {
      arr.push(secondStr[index]);
    } else arr.push(str[i]);
  }
  return arr.join('');
}
...
function getCardId(value) {
  ...
  const suitStr = value.slice(-1);
  const rankStr = value.slice(0, -1);
  const suitIndex = suitCard.indexOf(suitStr);
  const rankIndex = numberCard.indexOf(rankStr);
